package com.ywf.ywfpicturebackend.domain.picture.service.impl;
...
import com.ywf.ywfpicturebackend.infrastructure.manager.TxYunCosManager;
import com.ywf.ywfpicturebackend.infrastructure.manager.upload.PictureUploadTemplate;
import com.ywf.ywfpicturebackend.infrastructure.manager.upload.TxYunFilePictureUpload;
import com.ywf.ywfpicturebackend.infrastructure.manager.upload.TxYunUrlPictureUpload;
import com.ywf.ywfpicturebackend.infrastructure.utils.ColorSimilarUtils;
...
//            }
        ...
        if (pictureId != null) {
            ...
            // 执行查询
            Picture oldPicture = pictureRepository.getOne(queryWrapper);
...
//            }
        ...
        try {
            ...
            List<Picture> samePictures = pictureRepository.lambdaQuery().eq(Picture::getMd5, md5).eq(Picture::getSpaceId, spaceId).list();
        ...
        transactionTemplate.execute(status -> {
            boolean result = pictureRepository.save(finalPicture);
    ...
    public void doPictureReview(PictureReviewRequest pictureReviewRequest, User loginUser) {
        ...
        // 判断是否存在
        Picture oldPicture = pictureRepository.lambdaQuery().eq(Picture::getSpaceId, pictureReviewRequest.getSpaceId()).eq(Picture::getId, id).one();
        ...
        int result = pictureRepository.updatePictureReviewStatus(updatePicture);
    ...
    public void clearPictureFile(Picture oldPicture) {
        ...
        long count = pictureRepository.lambdaQuery().eq(Picture::getUrl, pictureUrl).eq(Picture::getSpaceId, oldPicture.getSpaceId()).count();
        // 有不止一条记录用到了该图片，不清理
        if (count > 1) {
            return;
        }
    ...
    public boolean editPicture(PictureEditRequest pictureEditRequest, User loginUser) {
        ...
        // 判断是否存在
        Picture oldPicture = pictureRepository.lambdaQuery().eq(Picture::getId, pictureEditRequest.getId()).eq(Picture::getSpaceId, picture.getSpaceId()).one();
        ThrowUtils.throwIf(oldPicture == null, ErrorCode.NOT_FOUND_ERROR);
        // 补充审核参数
        ...
        boolean result = pictureRepository.update(updateWrapper);
        ThrowUtils.throwIf(!result, ErrorCode.OPERATION_ERROR);
        return true;
    }
    ...
    public void editPictureByBatch(PictureEditByBatchRequest pictureEditByBatchRequest, User loginUser) {
        ...
        // 3. 查询指定图片，仅选择需要的字段
        List<Picture> pictureList = pictureRepository.lambdaQuery().select(Picture::getId, Picture::getSpaceId).eq(Picture::getSpaceId, spaceId).in(Picture::getId, pictureIdList).list();
        ...
        int updated = 0;
        for (Picture picture : pictureList) {
            updated += pictureRepository.updateByShardingKey(picture);
        }
        // 5. 批量更新
    ...
    public CreateOutPaintingTaskResponse createPictureOutPaintingTask(CreatePictureOutPaintingTaskRequest createPictureOutPaintingTaskRequest, User loginUser) {
        ...
        Picture picture = Optional.ofNullable(pictureRepository.lambdaQuery().eq(Picture::getSpaceId, spaceId).eq(Picture::getId, pictureId).one()).orElseThrow(() -> new BusinessException(ErrorCode.NOT_FOUND_ERROR));
        // 权限校验
    ...
    public Picture getById(Long id, Long spaceId) {
        return pictureRepository.lambdaQuery().eq(Picture::getId, id).eq(Picture::getSpaceId, spaceId).one();
    }
    ...
    public boolean updateById(Picture picture, HttpServletRequest request) {
        ...
        fillReviewParams(picture, loginUser);
        // 判断是否存在
        Picture isExists = getById(picture.getId(), picture.getSpaceId());
        ThrowUtils.throwIf(isExists == null, ErrorCode.NOT_FOUND_ERROR);
        // 操作数据库
        int result = pictureRepository.updateByShardingKey(picture);
